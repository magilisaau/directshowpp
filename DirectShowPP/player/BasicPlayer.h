...
private:
	...
	HWND m_hWnd;
public:
	void Open(HWND hWnd, TCHAR* pszFileName);
	CBasicPlayer();
	~CBasicPlayer(void);
    ...
    CSpectrumAnalyser* GetSpectrumAnalyser();
	void Start();
	void Stop();
	...
	DWORD getTime(void);
	//LARGE_INTEGER getPosition(void);
	int SetPosition(LARGE_INTEGER pos);
	LARGE_INTEGER getSize(void);
	BOOL isPause(void);
};
#endif