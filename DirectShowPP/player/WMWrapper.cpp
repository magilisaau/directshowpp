...
//	hr = reader->pWMSyncReader->OpenStream(reader->pStream);
	...
	for(i = 0; i < dwOutputCount; i++)
	{
		...
		if(IsEqualGUID(pOutputMediaType->majortype, WMMEDIATYPE_Audio))
		{
			...
			reader->pWMSyncReader->SetOutputSetting(i, L"EnableDiscreteOutput",
			...
			else
			{
				...
				{
					dwValue = iSpeakers;
					reader->pWMSyncReader->SetOutputSetting(i, L"SpeakerConfig",
	...
	if(reader->bHasAudio)
	{
		...
		if(hr == S_OK)
		{
			hr = reader->pWMHeaderInfo->GetAttributeByName(&wAStream, L"Duration", &dataType,
...
void WMA_Reader_GetAuthor(WMA_SYNC_READER* reader, WCHAR* pszAuthor)
{
	...
	WORD wLen = WMA_Reader_GetTagLength(reader, L"Author");
	if(wLen == 0) return;
...
DWORD WMA_Reader_GetBitrate(WMA_SYNC_READER* reader)
{
	...
	if(reader->pWMHeaderInfo->GetAttributeByName(&stream, L"Bitrate", &dataType, (BYTE*)&ret, &len) != S_OK)
		return 0;
...
DWORD WMA_Reader_GetFormatCount(WMA_SYNC_READER* reader, BOOL Descrete)
{
	...
	DWORD dwCount = 0;
	reader->pWMSyncReader->SetOutputSetting(reader->dwOutput, L"EnableDiscreteOutput",
...
BOOL WMA_Reader_GetIsVBR(WMA_SYNC_READER* reader)
{
	...
	stream = 0;
	BYTE ret;
	if(reader->pWMHeaderInfo->GetAttributeByName(&stream, L"", &dataType, &ret, &len) != S_OK)
		return FALSE;
...
{
	HRESULT rt;
	rt =reader->pWMSyncReader->SetRange((QWORD)(offset*0.1E6), 0);
}
